"""
RIQE — Single entry point for the full workflow.
────────────────────────────────────────────────
Pipeline + FastAPI app in one place. All paths come from riqe.config (unchanged).

  Run API server:    python -m riqe.app   OR   uvicorn riqe.app:app --host 0.0.0.0 --port 8000
  Use pipeline:     from riqe.app import create_pipeline; pipe = create_pipeline()
"""
...
# ── Pipeline (single instance when running the app) ────────────────────────

_pipeline: RIQEPipeline | None = None


def create_pipeline() -> RIQEPipeline:
    """Create and return a new pipeline. Use for scripts (e2e, tests) that need their own instance."""
    return RIQEPipeline()
...
async def _lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    global _pipeline
    _pipeline = RIQEPipeline()
    _pipeline.metrics.set_total_topics(len(_pipeline.topics))
    yield
...
# ── Run server when executed as module ─────────────────────────────────────

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("riqe.app:app", host="0.0.0.0", port=8000, reload=False)
